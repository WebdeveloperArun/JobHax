...
export default async function CompanyProfilePage() {
  ...
  const {
    ...
    defaultValues: {
      companyName: employer?.employerDetails.name || "My Company",
      tagline: employer?.employerDetails.metadata?.tagline || "Building the future of technology",
      description: employer?.employerDetails.description || "My Company is a leading technology company...",
      industry: employer?.employerDetails.industry || "technology",
      teamSize: employer?.employerDetails.teamSize || "500-1000",
      yearOfEstablishment: employer?.employerDetails.yearOfEstablishment || 2015,
      location: employer?.employerDetails.location || "San Francisco, CA",
      websiteUrl: employer?.employerDetails.websiteUrl || "https://mycompany.com",
      linkedinUrl: employer?.employerDetails.metadata?.socialLinks?.linkedin || "https://linkedin.com/company/mycompany",
      twitterUrl: employer?.employerDetails.metadata?.socialLinks?.twitter || "@mycompany",
      culture: employer?.employerDetails.metadata?.culture || "At My Company, we believe in fostering a collaborative environment...",
      benefits: employer?.employerDetails.metadata?.benefits || "• Competitive salary\n• Health insurance\n• Remote-friendly",
    },
  });
  ...
  const onSubmit = (data: CompanyProfileData) => {
    ...
    const result = updateEmployerProfile(payload);
